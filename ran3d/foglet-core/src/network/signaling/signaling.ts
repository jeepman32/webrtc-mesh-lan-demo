  ...
  constructor(source, options) {
    ...
    if (!source || !options || !options.address || !options.room) {
      ...
      throw new SyntaxError(
        "Not enough parameters, need a source an address and a room.",
      );
    }
    this.options = lmerge(
      {
  ...
  connection(network = null, timeout = this.options.timeout) {
    if (network === null && this._socket === null) {
      return Promise.reject(
        new Error(
          "There is no available connection to the server. Try to use the function signaling() before.",
        ),
      );
    }
    return new Promise((resolve, reject) => {
      ...
      try {
        if (network) {
          this._source
            .join(this.direct(this._network.rps, network.rps))
            .then(() => {
              done();
            })
            .catch(handleError);
